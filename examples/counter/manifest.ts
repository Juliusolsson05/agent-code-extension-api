...
export default {
  ...
  activationEvents: [
    'onCommand:managed.increment', 'onCommand:managed.read',
    'onCommand:managed.write', 'onCommand:managed.notify', 'onView:managed.main',
  ],
  permissions: ['fs.read', 'fs.write', 'notifications.show'],
  contributes: {
    commands: ['increment', 'snapshot', 'arm', 'read', 'write', 'notify']
      .map(id => ({ id: `managed.${id}`, title: id })),
